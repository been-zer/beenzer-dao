...
export const getWalletNFTs = async (
  ...
  const rawNFTs = await metaplex.nfts().findAllByOwner({owner}) as Array<any>;
  const NFTs:NFT[] = [];
  ...
  for ( const rawNFT of rawNFTs ) {
    ...
    const nftRow = {
      ...
      creators: rawNFT.creators.map((x:{address:string; share:number;}) => { 
        return { pubkey: x.address, share: x.share } }) as Creator[],
      name: metadata.name as string,
      description: metadata.description as string,
      asset_uri: metadata.image as string,
      metadata: metadata.attributes.map((x:{trait_type:string; value:number;}) => { 

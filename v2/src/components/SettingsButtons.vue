...
<div class="flex justify-center 2xl:ml-8 p-4">
    ...
    @click="() => {}" 
    :class="dark ? 'bg-white/10 shadow-gray-700 border-white/20 hover:bg-gray-600 text-white' : 'bg-white hover:bg-gray-200 border-gray-100 text-gray-600'">
      <img v-if="sound" :src="dark ? sound_white : sound_black" class="h-4 w-4"/>
      <img v-else :src="dark ? mute_white : mute_black" class="h-4 w-4"/>
    </button>
  <!-- Dark Button -->
  <button class="rounded-full h-8 w-8 m-2 flex justify-center items-center shadow-xl border " 
  @click="() => {}" 
